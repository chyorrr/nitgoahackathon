...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      console.error('Login failed:', { error: errorMsg, code: error.status })
      return
    }
    ...
    // Persist minimal client-side state if you rely on it
    try {
      // Get fresh user data and store it
      const { data: { user } } = await supabase.auth.getUser()
      ...
      if (user) {
        ...
        // Store fresh data, using metadata from signup
        localStorage.setItem('isLoggedIn', 'true')
        const displayName = user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split('@')[0] || ''
        localStorage.setItem('userName', displayName)
        localStorage.setItem('userEmail', user.email || '')
        
        console.log('Stored fresh user data:', {
          raw_metadata: user.user_metadata,
          stored_name: displayName,
          email: user.email
        })
      }
    } catch (e) {
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="space-y-2.5">
                {/* CAPTCHA Display */}
                <div className="bg-linear-to-r from-slate-100 to-slate-200 rounded-lg p-3 border border-slate-300">
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center gap-2 cursor-pointer group">
                <input
                  type="checkbox"
                  className="w-4 h-4 rounded border-[#E0E0E0] text-[#2979FF] focus:ring-[#2979FF] transition-all"
                />
                <span className="text-slate-600 group-hover:text-slate-900 transition-colors">Remember me</span>
              </label>
              <Link href="/forgot-password" className="text-[#00C853] hover:text-[#69F0AE] transition-colors font-medium">
                Forgot password?
              </Link>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              onClick={async () => {
                // redirect back to our OAuth callback page after provider auth
                await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${window.location.origin}/auth/callback` } })
              }}
              className="flex items-center justify-center gap-2 px-4 py-2.5 border border-slate-300 rounded-lg hover:bg-slate-50 transition-all duration-300 group"
            >
          ...
          <div className="mt-5 text-center text-sm text-slate-600">
            ...
            <Link href="/signup" className="text-[#00C853] hover:text-[#69F0AE] font-semibold transition-colors">
              Sign up for free
            </Link>
          </div>
        </motion.div>
